...
public class MatrixUtils {
    ...
    /**
     * helper function get that obtains a value in a 2d
     * array and returns infinity if the array parameters
     * are not valid. */
    ...
    public static double[][] accumulateVertical(double[][] m) {
        double[][] answer = MatrixUtils.copy(m);
        for (int r = 1; r < m.length; r++) {
            for (int c = 0; c < m[r].length; c++) {
                int right = c + 1;
                int left = c - 1;
                int row = r - 1;
                answer[r][c] += Math.min(Math.min(get(answer, row, right), get(answer, row, left)),
                        get(answer, row, c));
            }
        }
        return answer;
    ...
    /** Helper function for accumulate that transposes
     * a matrix. */
