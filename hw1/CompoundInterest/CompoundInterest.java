...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate, int targetYear) {
        return (presentValue * Math.pow((1.0 + (rate / 100.0)), numYears(targetYear)));
    }
    ...
    static double futureValueReal(double presentValue, double rate, int targetYear, double inflationRate) {
       return (presentValue * Math.pow((1.0 + (rate / 100.0)), numYears(targetYear)) * Math.pow((1 - (inflationRate / 100)), numYears(targetYear)));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        int x = numYears(targetYear);
        double answer = 0.0;
        while (x >= 0) {
            answer += (perYear * Math.pow((1.0 + (rate / 100.0)), x));
            x -= 1;
        }
        return answer;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate, double inflationRate) {
        return totalSavings(perYear, targetYear, rate) * Math.pow((1 - (inflationRate / 100.0)), numYears(targetYear));
    }
