...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        int row = place.row;
        int column = place.col;
        if (isCell(row + 1, column)) {
            action.accept(Place.pl(row + 1, column));
        }
        if (isCell(row - 1, column)) {
            action.accept(Place.pl(row - 1, column));
        }
        if (isCell(row, column + 1)) {
            action.accept(Place.pl(row, column + 1));
        }
        if (isCell(row, column - 1)) {
            action.accept(Place.pl(row, column - 1));
        }
    ...
    boolean[] colorsPresent() {
        boolean[] answer = new boolean[_ncolors];
        for (int rows = 0; rows < _height; rows++) {
            for (int columns = 0; columns < _width; columns++) {
                answer[get(rows, columns)] = true;
            }
        } return answer;
    }
    ...
    final boolean solved() {
        return (activeRegionSize() == _width * _height);
    }
    ...
    HashSet<Place> findRegion(Place start, int color, HashSet<Place> result) {
        Consumer<Place> colortest = (box) -> {
            if (result.contains(box)) {
                return;
            } else {
                if (get(box) == color) {
                    result.add(box);
                    findRegion(box, color, result);
                }
            }
        };
        result.add(start);
        forNeighbors(start, colortest);
        return result;
    }
    ...
    void setActiveRegionColor(int newColor) {
        HashSet<Place> newactive = new HashSet<>();
        for (Place place: _active) {
            _cells[place.row][place.col] = newColor;
            newactive = findRegion(place, newColor, newactive);
        }
        _active = newactive;
        _current += 1;
        ...
        if (_current == _history.size()) {
            ...
            _history.set(_current, new GameState());
        }
    ...
    Set<Place> adjacentCells(int color) {
        HashSet<Place> answer = new HashSet<>();
        for (Place place: _active) {
            answer = findRegion(place, color, answer);
        }
        for (Place place: _active) {
            answer.remove(place);
        }
